...
#published: false
---
یک روش کمی برای حل زمان روزهای طولانی
<!--more-->
...
```python
import branca
colorscale = branca.colormap.linear.Paired_07.scale(vmin=9, vmax=24)
...
def style_by_duration_delta(feature):
    i = feature['properties']['ISO_A3']
    try:
        dur = df.set_index('cca3').loc[i,'duration_b']
    except KeyError:
...
```python
m = folium.Map(location=[20, 10],zoom_start=2,max_bounds=True,
               width=750, height=500,
               crs='EPSG3857', tiles='cartodb positron')

folium.GeoJson(data, style_by_duration, highlight_function, name="default",).add_to(m)
folium.GeoJson(data, style_by_duration_b, highlight_function, name="blended").add_to(m)
